              ...
              <section class="vbox">
                ...
                <section class="panel panel-default">
                  <header class="panel-heading">
                    Announcement data <span class='badge bg-info'><?=$jumlah;?></span>
                  </header>
              <form method="POST" id="cek-form" name="action" action="<?=base_url('administrator/announcement/action');?>"> 
                <div class="row wrapper">
                  <div class="col-sm-5 m-b-xs">
                    <select class="input-sm form-control input-s-sm inline v-middle" id='pilih' name='pilih'>
                     ...
                     <a href="<?=base_url('administrator/announcement/add');?>" class="btn btn-sm btn-primary"> <i class="fa fa-plus"></i> Add Announcement</a>
                  </div>
                  ...
                  ?>
                    <div class="btn-group" data-toggle="buttons">
                      ...
                      <label class="btn btn-sm btn-default <?=$all;?>" onclick="redirect_post('<?=base_url('administrator/announcement?v=all');?>')" >
                        All
                      </label>
                      <label class="btn btn-sm btn-default <?=$draf;?>" onclick="redirect_post('<?=base_url('administrator/announcement?v=draf');?>')">
                        <input type="radio" name="options" id="option2" > Draf
                      </label>
                      <label class="btn btn-sm btn-default <?=$pending;?>" onclick="redirect_post('<?=base_url('administrator/announcement?v=pending');?>')">
                       Pending
                      </label>
                      <label class="btn btn-sm btn-default <?=$trash;?>" onclick="redirect_post('<?=base_url('administrator/announcement?v=trash');?>')">
                        Trash
                      </label>
                      <label class="btn btn-sm btn-default <?=$your_post;?> " onclick="redirect_post('<?=base_url('administrator/announcement?v=your_post');?>')">
                        Your post
                      </label>
                    </div>
                  </div>
                  <div class="col-sm-3">
                    ...
                    <input type="hidden" value="<?=@$_GET['v'];?>" id="view">
                      ...
                      <span class="input-group-btn">
                        <button class="btn btn-sm btn-default" onclick="cari('announcement')" type="button">Go!</button>
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div class="table-responsive">
                  <table class="table table-striped b-t b-light">
                    ...
                    <tr>
                      ...
                      <br> ";
                      if($kode_login == $user || $jabatan=="Administrator" ){
                        echo "<small class='clear pull-left'><a href='".base_url('administrator/announcement/edit/'.$kode)."'><span class='text-primary'>edit</a></span> | <a href='".base_url('administrator/announcement/delete/'.$kode)."'><span class='text-danger'>delete</span></a>
                        | <a href='".base_url('administrator/announcement/eng_post/'.$kode.'/?k=announcement')."'><span class='text-warning'>english post</span></a></small>
                      ";
                      }
                      if($jabatan=="Administrator" && $statusx=="pending"){
                        echo " | <a href='".base_url('administrator/announcement/update_pending/'.$kode)."'><span class='text-info'>approve</a></span></small>";
                      }
                      echo"

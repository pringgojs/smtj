...
class Agenda extends CI_Controller {
   ...
   function insert_aktifitas($user, $keterangan){
     ...
     function index(){
          ...
          //pagination settings
        $config['base_url'] = base_url('administrator/agenda/');
        $config['total_rows'] = $this->db->count_all('agenda_view');
        $config['per_page'] = "10";
        ...
        $this->pagination->initialize($config);
          ...
          $data['post'] = $this->modelnews->data_limit("agenda_view",$where,'id_post',$config["per_page"],$data['page'])->result();
          ...
          $this->load->view('admin/agenda/main',$data);
     ...
     function save(){
          ...
          $this->db->update('tb_post', $data); 
	       $this->insert_aktifitas($kode, "Menambah agenda '".$judul."' ");
          redirect('administrator/agenda');
	...
	$user = $this->cekLogin();
	 $this->insert_aktifitas($user, "Mengubah status agenda");
          redirect('administrator/agenda');
     }
	...
	$user = $this->cekLogin();
        $this->insert_aktifitas($user, "Menghapus agenda dengan ID : '".$kode."' ");
        redirect('administrator/agenda');
    }
  ...
  function cek_modul_id($kode=""){
    ...
    function cek_hak_akses_user(){
      $kode_secure= 'x0014'; //  kode halaman user
      $t = $this->cekLogin(); // get kode login
